  ...
  fun runBenchmark(
    ...
    viewModelScope.launch(Dispatchers.Default) {
      ...
      if (model.isSystemManaged) {
        ...
        if (!initialized) {
          Log.e(TAG, "Failed to initialize system-managed model for benchmark")
        ...
        for (i in 0 until runCount) {
          Log.d(TAG, "Start running system-managed benchmark #$i...")
          ...
          try {
            val latency =
              suspendCancellableCoroutine<Long> { continuation ->
                ...
                model.runtimeHelper.runInference(
                  ...
                  cleanUpListener = {},
                  onError = { errMsg ->
                    Log.e(TAG, "System-managed benchmark run failed: $errMsg")
                    errorOccurred = true
            ...
            Log.e(TAG, "Exception during system-managed benchmark run", e)
            prefillSpeeds.add(0.0)
            decodeSpeeds.add(0.0)
            timesToFirstToken.add(0.0)
            if (i > 0) nonFirstInitTimes.add(0.0)
