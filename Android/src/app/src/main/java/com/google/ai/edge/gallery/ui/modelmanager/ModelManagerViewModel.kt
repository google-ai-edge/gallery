...
constructor(
  ...
  // TODO: b/494029782 - Both litertlm and aicore download and storage should be unified into a
  // model repository.
  private fun checkSystemManagedModelStatuses() {
    viewModelScope.launch(Dispatchers.Main) {
      val systemManagedModels =
        uiState.value.tasks
          .flatMap { it.models }
      ...
      for (model in systemManagedModels) {
  ...
  fun loadModelAllowlist() {
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        // Wait for system-managed models statuses and update download indicators
        checkSystemManagedModelStatuses()
      } catch (e: Exception) {
        e.printStackTrace()
      }
    }
