...
class EvalServerTest {
  ...
  @Test
  fun componentExecuteEndpoint_missingBody_returnsBadRequest() {
    val url = URL("http://localhost:$port/v1/components/123/execute")
    val connection = url.openConnection() as HttpURLConnection
    connection.requestMethod = "POST"
    connection.doOutput = true
  ...
  @Test
  fun componentExecuteEndpoint_validRequest_returnsOk() {
    // Inject Mock ModelManager
    val field = EvalServer::class.java.getDeclaredField("modelManager")
    field.isAccessible = true
    val fakeManager =
  ...
  @Test
  fun componentExecuteEndpoint_success() {
    val fakeManager =
      object : ModelManager(context) {
        override fun getHelperForModelName(modelName: String): LlmModelHelper? =
          FakeLlmModelHelper()
  ...
  @Test
  fun componentExecuteEndpoint_missingBody_badRequest() {
    val url = URL("http://localhost:$port/v1/components/123/execute")
    val connection = url.openConnection() as HttpURLConnection
    connection.requestMethod = "POST"
    connection.doOutput = true
